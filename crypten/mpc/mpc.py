...
class MPCTensor(CrypTensor):
    ...
    def relu(self):
        ...
        return MPCTensor.from_shares(
                resharing.mixed_mul(self._tensor, new_get_msb(self._tensor)), 
    ...
    def pairwise_max(self, y):
        ...
        comp_bit = new_get_msb((self-y)._tensor)
        toc = time.perf_counter()
        pairwise_max = resharing.new_mixed_mul((self-y)._tensor, comp_bit) + y
    ...
    def softmax(self, dim, **kwargs):
        ...
        #print(self.get_plain_text().sum())
        maximum_value = self.max(dim, keepdim=True)
        #print(maximum_value.get_plain_text().sum())
        logits = self - maximum_value
    
        numerator = logits.exp()
        #print(numerator.get_plain_text().sum())
