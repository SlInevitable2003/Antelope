...
"""
def max_pool2d(self, kernel_size, padding=0, stride=None, return_indices=True):
    ...
    #临时降低维度
    #print(tensor_torch._tensor.shape)
    ...
    #print(tensors._tensor.shape,tensor_torch._tensor.shape)
    ...
    while tensors.size(0) > 1:
        ...
        sign_bit = new_get_msb(diff)
        
        max, max_index = compare_protect(sign_bit,x,y,x_i, y_i)
        #print(max.get_plain_text(0))
...
def compare_protect(sign_bit,max,cur,index_max,index_cur):
    ...
    copy_bit=sign_bit.clone()
    value = resharing.new_mixed_mul((max-cur), sign_bit) + cur
    index = resharing.new_mixed_mul((index_max-index_cur), copy_bit) + index_cur
    
    return value._tensor,index._tensor
...
def max_pool2d3242(self, kernel_size, padding=0, stride=None, return_indices=True):
    import crypten
    # import os
    ...
    mpc_out, mpc_indices = max_pool2d_2(x,kernel_size=kernel_size,stride=stride)
