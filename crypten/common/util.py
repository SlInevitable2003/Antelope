...
def get_im2col_indices_cuda(x_shape, field_height, field_width, padding=1, stride=1):
    ...
    i0 = torch.repeat_interleave(torch.arange(field_height,device='cuda'), field_width)
    i0 = i0.repeat(C)

    i1 = stride * torch.repeat_interleave(torch.arange(out_height,device='cuda'), out_width)
    j0 = torch.arange(field_width,device='cuda').repeat(field_height * C)

    j1 = stride * torch.arange(out_width,device='cuda').repeat(out_height)
    i = i0.reshape(-1, 1) + i1.reshape(1, -1)
    j = j0.reshape(-1, 1) + j1.reshape(1, -1)

    k = torch.repeat_interleave(torch.arange(C,device='cuda'), field_height * field_width).reshape(-1, 1)
...
def im2col(x, filter_h, filter_w, padding=1, stride=1):
    ...
    out_h = (H + 2*p - filter_h)//stride + 1
    # 对图像进行卷积运算后的输出宽度
    out_w = (W + 2*p - filter_w)//stride + 1

	# 对图像在4个维度进行填充，默认pad为0
    img = F.pad(x.share, (p, p, p, p), mode='constant')
    
    # 见下面解释1
    col = torch.zeros([N, C, filter_h, filter_w, out_h, out_w], dtype=torch.int64, device='cuda')
   ...
   # 从左到右，从上到下依次进行遍历
    ...
    #print("\ncol.transpose res is:\n",col.permute(0, 4, 5, 1, 2, 3))
    col = col.permute(0, 4, 5, 1, 2, 3).reshape(N*out_h*out_w, -1)
    from crypten.mpc import MPCTensor
    return MPCTensor.from_shares(col, src=comm.get().get_rank())
...
def im2col_indices(x, field_height, field_width, padding=1, stride=1):
    ...
    k, i, j = get_im2col_indices(x.shape, field_height, field_width, padding,
                                stride)
    #print(x_padded.shape,torch.from_numpy(k).shape,torch.from_numpy(i).shape,torch.from_numpy(j).shape)
