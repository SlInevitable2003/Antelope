...
def timer_relu(func):
    ...
    def wrapper_timer(*args, **kwargs):
        ...
        value = func(*args, **kwargs)
        end_time = time.perf_counter()      # 2
        run_time = end_time - start_time    # 3  111ms
...
class AutogradReLU(AutogradFunction):
    ...
    def forward(ctx, input):
        ...
        from crypten.mpc.primitives.converters import get_msb,new_get_msb
        from crypten import communicator as comm
        import time
        rank = comm.get().get_rank()
    
        mask = new_get_msb(input._tensor)
        ctx.save_for_backward(mask)

        a = resharing.new_mixed_mul(input._tensor, mask)
    ...
    def backward(ctx, grad_output):
        ...
        return resharing.new_mixed_mul(grad_output._tensor, mask)
...
class AutogradAvgPool2D(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        #print(grad_output.get_plain_text().flatten(),grad_output.get_plain_text().flatten().sum(-1))
        #print(input_size,grad_output.shape)
        grad_out_reshaped = grad_output.permute(1, 2, 3, 0).reshape(c, 1, -1)
        kernel = crypten.cryptensor(torch.ones(c, ky*kx, 1)/ (ky * kx), device='cuda', requires_grad=False)

        #grad_input_fold = kernel.matmul(grad_out_reshaped)
        grad_input_fold = torch.matmul(kernel.get_plain_text(), grad_out_reshaped.get_plain_text())
        grad_input_fold = crypten.cryptensor(grad_input_fold, device='cuda', requires_grad=False)
...
class AutogradConv2D(AutogradFunction):
    ...
    def forward(ctx, input, kernel, padding=0, stride=1, groups=1):
        ...
        a = input.conv2d(kernel, padding=padding, stride=stride, groups=groups)
        return a
    ...
    def backward(ctx, grad_output):
        ...
        grad_kernel = input.conv2d_back(grad_out_reshaped,kernel=kernel,padding=padding, stride=stride)
...
class AutogradBatchNorm(AutogradFunction):
    ...
    def forward(
        ...
        """
        # print(weight.get_plain_text().flatten())
        import time
        # read running params
        running_mean = bn_param['running_mean']
        inv_running_var = bn_param['inv_running_var']
        ...
        if training:
            ...
            div_data = crypten.cryptensor(1/size, device='cuda', requires_grad=False)

            mean = input.sum(-1) * div_data
            input_sub = (input.transpose(0,1) - mean).transpose(0,1)
            var = ((input_sub * input_sub).sum(-1)) * div_data + eps
            rank = comm.get().get_rank()
            input_tensor = var.share._tensor
            if rank==0:
                var_tensor = (input_tensor - 1).reshape(broadcast_shape[1],-1)
            else:
                var_tensor = (input_tensor).reshape(broadcast_shape[1],-1)
            for i in range(1,24):
                if rank==0:
                    var_tensor = torch.cat((var_tensor,(input_tensor-(1<<i)).reshape(broadcast_shape[1],-1)),1)
                else:
                    var_tensor = torch.cat((var_tensor,input_tensor.reshape(broadcast_shape[1],-1)),1)
            
            from crypten.mpc.primitives.arithmetic import ArithmeticSharedTensor
            from crypten.mpc.primitives import resharing
            from crypten.mpc.primitives.converters import new_get_msb, get_msb
            output_sub = ArithmeticSharedTensor.from_shares(var_tensor, src=rank)
            mask = get_msb(output_sub)^1
            
            result = mask.get_plain_text()
            alpha = result._tensor.sum(-1)
            alpha = alpha//2
            ini_value = torch.tensor([2],dtype=torch.int64,device='cuda')**alpha
            inv_var = crypten.cryptensor(1024/ini_value,src=0)
            it_num = 4
            for i in range(it_num):
                inv_var = (inv_var/2)*3 - (var*inv_var*inv_var*inv_var)/2
            # save running
            running_mean = momentum * running_mean + (1 - momentum) * mean
            inv_running_var = momentum * inv_running_var + (1 - momentum) * inv_var

            bn_param['running_mean'] = running_mean
            bn_param['inv_running_var'] = inv_running_var 
            ...
            inv_var = bn_param['inv_running_var']
            # reshape shape (C) to broadcastable (1, C, 1, +):
            mean = mean.reshape(broadcast_shape)
            inv_var = inv_var.reshape(broadcast_shape)
    ...
    def backward(ctx, grad_output):
        ...
        #grad_output = grad_output*1024*1024
        # determine dimensions over which means and variances are computed:
        stats_dimensions = list(range(len(grad_output.shape)))
        stats_dimensions.pop(1)
        # print("bn",grad_output.get_plain_text().flatten())
        ...
        #BNprint(grad_output.shape,x_norm.shape)
...
class AutogradCrossEntropy(AutogradFunction):
    ...
    def forward(ctx, pred, target, skip_forward=False):
        ...
        #print(softmax.get_plain_text().sum()/softmax.numel(),softmax.get_plain_text().flatten())
        # Compute full forward pass
        loss_values = softmax.log().mul_(target).neg_()
        #print(loss_values.get_plain_text().sum())
...
class AutogradMaxPool2D(AutogradFunction):  
    ...
    def backward(ctx, grad_output):
        ...
        if isinstance(out, CUDALongTensor):
            out = out.tensor()
        #print(output_size,out.shape,indices.shape)
