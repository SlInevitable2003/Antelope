...
class CUDALongTensor(object):
    ...
    def matmul(x, y, *args, **kwargs):
        ...
        if x.dim()==2 and y.dim()==2:
            ...
            n = x.shape[1]
            k = y.shape[1]
            z_new = torch.randint(0,1,[m,k],device='cuda')
            import mat_mul
            x = x._tensor.contiguous()
            y = y._tensor.contiguous()
            mat_mul.torch_launch_matmul(z_new, x, y, m, n, k)
            
            return CUDALongTensor(z_new)
        else:
    ...
    def conv1d(input, kernel, *args, **kwargs):
        return CUDALongTensor.__patched_conv_ops(
            "conv1d", input, kernel, *args, **kwargs
        )
    ...
    def conv2d(input, kernel, *args, **kwargs):
        ...
        FN, C, KH, KW = kernel.shape
        N, C, H, W = input.shape

        tic = time.perf_counter()
        input_fold = im2col(input, KH, KW, padding=padding, stride=stride)

        toc = time.perf_counter()

        x = input_fold.share
        y = kernel.reshape(kernel.shape[0],-1).transpose(0,1)

        tic1 = time.perf_counter()
        z = getattr(torch, "matmul")(x, y)

        toc1 = time.perf_counter()
    
        z = z.reshape(N, z.shape[0]//N, -1).transpose(1,2)
        out_h = (H - KH + 2*padding[0]) // stride[0] + 1
        z = z.reshape(N, FN, out_h, -1)
        
        z._tensor=z._tensor.contiguous()
        return z
